"""OTR 1.0 endpoint-label baseline (RETAINED FOR BENCHMARKING ONLY).

This is the original net-increment formulation with the DEFECTIVE
endpoint overflow label Y = 1{W_m > B} — the predecessor that OTR-2.0
supersedes (see core/otr2.py Proposition 1 discussion). The maintained
threshold policy now lives in core/otr.py (load-space, peak label,
contributed rewrite). This module exists so the paper's v1-endpoint
comparison column stays reproducible. Do not use it in new code.
