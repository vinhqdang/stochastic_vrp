...
Resume an interrupted run (same CLI/settings and output prefix):
  python go_nogo_orl_certified_dethloff_v4_full_alns.py full dir=Dethloff resume=1 overwrite=0
...
and a full classical ALNS layer directly into this file. It does not import the old
solver or code-orl-extend.txt. The ALNS uses random/worst removal, greedy/regret-2
repair, adaptive roulette-wheel weights, simulated-annealing acceptance, and optional
2-opt/relocate local search.
...
Quick research decision (recommended first):
    ...
    python go_nogo_orl_certified_dethloff_v4_full_alns.py quick ^
      dir="C:\\path\\to\\Dethloff"

Very small sanity run:

    python go_nogo_orl_certified_dethloff_v4_full_alns.py smoke ^
      dir="C:\\path\\to\\Dethloff"

Full benchmark:

    python go_nogo_orl_certified_dethloff_v4_full_alns.py full ^
      dir="C:\\path\\to\\Dethloff"
...
# Classical ALNS defaults, matching the user's existing ALNS implementation.
...
class ALNSParameters:
    destroy_min_frac: float = ALNS_DESTROY_MIN_FRAC
    destroy_max_frac: float = ALNS_DESTROY_MAX_FRAC
    sa_temp: float = ALNS_SA_TEMP
    sa_cool: float = ALNS_SA_COOL
    segment_size: int = ALNS_SEGMENT_SIZE
    reaction: float = ALNS_REACTION
    sigma_1: float = ALNS_SIGMA_1
    sigma_2: float = ALNS_SIGMA_2
    sigma_3: float = ALNS_SIGMA_3
    worst_determinism: float = ALNS_WORST_DETERMINISM
    weight_floor: float = ALNS_WEIGHT_FLOOR
    use_local_search: bool = True

    def validate(self) -> None:
        if not (0.0 < self.destroy_min_frac <= self.destroy_max_frac < 1.0):
            raise ValueError("destroy fractions must satisfy 0 < min <= max < 1")
        if self.sa_temp <= 0.0:
            raise ValueError("sa_temp must be positive")
        if not (0.0 < self.sa_cool <= 1.0):
            raise ValueError("sa_cool must lie in (0,1]")
        if self.segment_size <= 0:
            raise ValueError("segment_size must be positive")
        if not (0.0 <= self.reaction <= 1.0):
            raise ValueError("reaction must lie in [0,1]")
        if min(self.sigma_1, self.sigma_2, self.sigma_3) < 0.0:
            raise ValueError("operator scores must be nonnegative")
        if self.worst_determinism <= 0.0:
            raise ValueError("worst_determinism must be positive")
...
class ALNSRunResult:
    ...
    trace_digest: str
    accepted_moves: int
    accepted_nonimproving: int
    new_bests: int
    destroy_usage: tuple[int, int]
    repair_usage: tuple[int, int]
    final_destroy_weights: tuple[float, float]
    final_repair_weights: tuple[float, float]


def cw_init(distance: np.ndarray, gate: FullGate | CertifiedGate, n: int) -> list[list[int]]:
    routes = [[customer] for customer in range(1, n) if gate.feasible([customer])]
    ...
    for customer in leftovers:
        # A customer that is infeasible even alone makes the instance infeasible
        # under the selected gate.  Do not silently inject an infeasible route.
        if not gate.feasible([customer]):
            raise ValueError(f"customer {customer} is infeasible as a singleton route")
    ...
    while improved:
        ...
        for route_index in range(len(solution)):
            ...
            for position in range(len(route)):
                ...
                removal_gain = distance[a, customer] + distance[customer, b] - distance[a, b]
                for target_index in range(len(solution)):
                    ...
                    for insertion_position in range(len(target) + 1):
                        ...
                        delta = (
                            distance[u, customer] + distance[customer, v]
                            - distance[u, v] - removal_gain
                        )
                        if delta < -1e-9:
                            new_route = route[:position] + route[position + 1:]
                            new_target = target[:insertion_position] + [customer] + target[insertion_position:]
...
def local_search(
    ...
    solution = [two_opt_gate(route, distance, gate) for route in solution if route]
...
def _removal_count(total_customers: int, rng: random.Random, params: ALNSParameters) -> int:
    if total_customers <= 0:
        return 0
    fraction = rng.uniform(params.destroy_min_frac, params.destroy_max_frac)
    return min(total_customers, max(1, int(round(fraction * total_customers))))
...
def random_removal(
    ...
    del distance, omega_vehicle
    working = [list(route) for route in solution if route]
    customers = [customer for route in working for customer in route]
    q = _removal_count(len(customers), rng, params)
    removed = rng.sample(customers, q) if q else []
    removed_set = set(removed)
    partial = [[customer for customer in route if customer not in removed_set] for route in working]
    return [route for route in partial if route], removed
...
def worst_removal(
    ...
    working = [list(route) for route in solution if route]
    total = sum(len(route) for route in working)
    q = _removal_count(total, rng, params)
    removed: list[int] = []

    for _ in range(q):
        ranked: list[tuple[float, int, int, int]] = []
        for route_index, route in enumerate(working):
            old_cost = route_cost(route, distance) + omega_vehicle
            for position, customer in enumerate(route):
                trial = route[:position] + route[position + 1:]
                new_cost = route_cost(trial, distance) + omega_vehicle if trial else 0.0
                ranked.append((old_cost - new_cost, route_index, position, customer))
        if not ranked:
            break
        ranked.sort(key=lambda item: (-item[0], item[3]))
        rank = min(
            int(len(ranked) * (rng.random() ** params.worst_determinism)),
            len(ranked) - 1,
        )
        _, route_index, position, customer = ranked[rank]
        removed.append(customer)
        del working[route_index][position]
        if not working[route_index]:
            del working[route_index]
...
def _insertion_options(
    ...
    for route_index, route_sequence in enumerate(solution):
        route = list(route_sequence)
        before = route_cost(route, distance)
        for position in range(len(route) + 1):
            candidate = route[:position] + [customer] + route[position:]
            if gate.feasible(candidate):
                delta = route_cost(candidate, distance) - before
                options.append((float(delta), route_index, position))
    singleton = [customer]
    if gate.feasible(singleton):
        options.append((route_cost(singleton, distance) + omega_vehicle, -1, 0))
    return options


def greedy_repair(
    partial: Sequence[Sequence[int]],
    ...
    solution = [list(route) for route in partial if route]
    pool = list(removed)
    rng.shuffle(pool)
    for customer in pool:
        options = _insertion_options(solution, customer, distance, gate, omega_vehicle)
        if not options:
            raise ValueError(f"no feasible insertion for customer {customer}")
        delta, route_index, position = min(options, key=lambda item: (item[0], item[1], item[2]))
        del delta
        if route_index == -1:
            solution.append([customer])
        else:
...
def regret2_repair(
    ...
    del rng  # deterministic once the destroy operator has selected the removed set
    solution = [list(route) for route in partial if route]
    pool = list(removed)
    while pool:
        ...
        for customer in pool:
            ...
            regret = float(second_cost - best[0])
            records.append((regret, float(best[0]), customer, best[1], best[2]))
        # Highest regret first; then cheapest best insertion; then customer id.
        regret, best_delta, customer, route_index, position = max(
            records,
            key=lambda item: (item[0], -item[1], -item[2]),
        )
        del regret, best_delta
        if route_index == -1:
            solution.append([customer])
        else:
            solution[route_index].insert(position, customer)
...
def _roulette_index(weights: Sequence[float], rng: random.Random) -> int:
...
def solve_alns(
    ...
    current = cw_init(distance, gate, n)
    if params.use_local_search:
        current = local_search(current, distance, gate)
    current = [route for route in current if route]
    current_cost = econ_cost(current, distance, omega_vehicle)
    best = [route[:] for route in current]
    best_cost = current_cost

    destroy_weights = [1.0, 1.0]
    repair_weights = [1.0, 1.0]
    segment_destroy_scores = [0.0, 0.0]
    segment_repair_scores = [0.0, 0.0]
    segment_destroy_counts = [0, 0]
    segment_repair_counts = [0, 0]
    total_destroy_usage = [0, 0]
    total_repair_usage = [0, 0]

    accepted_moves = 0
    accepted_nonimproving = 0
    new_bests = 0
    temperature = float(params.sa_temp)
    trace = hashlib.sha256()
    trace.update(repr(params).encode("utf-8"))

    for iteration in range(1, iterations + 1):
        destroy_index = _roulette_index(destroy_weights, rng)
        repair_index = _roulette_index(repair_weights, rng)
        total_destroy_usage[destroy_index] += 1
        total_repair_usage[repair_index] += 1
        segment_destroy_counts[destroy_index] += 1
        segment_repair_counts[repair_index] += 1

        partial, removed = destroy_ops[destroy_index](
            current, distance, omega_vehicle, rng, params
        )
        candidate = repair_ops[repair_index](
            partial, removed, distance, gate, omega_vehicle, rng
        )
        if params.use_local_search:
            candidate = local_search(candidate, distance, gate)
        ...
        accepted = improves_current
        accepted_nonimproving_this_iteration = False
        if not accepted:
            probability = math.exp(-max(delta, 0.0) / max(temperature, 1e-15))
            if rng.random() < probability:
                accepted = True
                accepted_nonimproving_this_iteration = True

        new_best_this_iteration = candidate_cost < best_cost - 1e-9
        if accepted:
            current = [route[:] for route in candidate]
            current_cost = candidate_cost
            accepted_moves += 1
            if accepted_nonimproving_this_iteration:
                accepted_nonimproving += 1

            if new_best_this_iteration:
                best = [route[:] for route in candidate]
                best_cost = candidate_cost
                new_bests += 1
                score = params.sigma_1
            elif improves_current:
                score = params.sigma_2
            else:
                score = params.sigma_3
            segment_destroy_scores[destroy_index] += score
            segment_repair_scores[repair_index] += score

        if iteration % params.segment_size == 0:
            for index in range(2):
                if segment_destroy_counts[index] > 0:
                    performance = segment_destroy_scores[index] / segment_destroy_counts[index]
                    destroy_weights[index] = max(
                        params.weight_floor,
                        params.reaction * destroy_weights[index]
                        + (1.0 - params.reaction) * performance,
                    )
                if segment_repair_counts[index] > 0:
                    performance = segment_repair_scores[index] / segment_repair_counts[index]
                    repair_weights[index] = max(
                        params.weight_floor,
                        params.reaction * repair_weights[index]
                        + (1.0 - params.reaction) * performance,
                    )
            segment_destroy_scores = [0.0, 0.0]
            segment_repair_scores = [0.0, 0.0]
        ...
        temperature = max(temperature * params.sa_cool, 1e-15)
        event = (
            f"{iteration}|{destroy_index}|{repair_index}|{len(removed)}|"
            f"{int(accepted)}|{int(accepted_nonimproving_this_iteration)}|"
            f"{int(new_best_this_iteration and accepted)}|"
            f"{current_cost:.12f}|{best_cost:.12f}|"
            f"{destroy_weights[0]:.12f}|{destroy_weights[1]:.12f}|"
            f"{repair_weights[0]:.12f}|{repair_weights[1]:.12f}\n"
        )
        trace.update(event.encode("utf-8"))

    return ALNSRunResult(
        plan=[route for route in best if route],
        trace_digest=trace.hexdigest(),
        accepted_moves=accepted_moves,
        accepted_nonimproving=accepted_nonimproving,
        new_bests=new_bests,
        destroy_usage=(total_destroy_usage[0], total_destroy_usage[1]),
        repair_usage=(total_repair_usage[0], total_repair_usage[1]),
        final_destroy_weights=(destroy_weights[0], destroy_weights[1]),
        final_repair_weights=(repair_weights[0], repair_weights[1]),
    )
...
def run_solver_once(
    ...
    result: dict[str, object] = {
        ...
        "solver": "ALNS",
        "search_trace_digest": search.trace_digest,
        "accepted_moves": search.accepted_moves,
        "accepted_nonimproving": search.accepted_nonimproving,
        "new_bests": search.new_bests,
        "destroy_usage": search.destroy_usage,
        "repair_usage": search.repair_usage,
        "final_destroy_weights": search.final_destroy_weights,
        "final_repair_weights": search.final_repair_weights,
    }
...
def assert_pair_identity(
    ...
    if full["search_trace_digest"] != cert["search_trace_digest"]:
        raise AssertionError(f"{context}: FULL/CERT ALNS trajectories differ")
    for field in [
        "accepted_moves", "accepted_nonimproving", "new_bests",
        "destroy_usage", "repair_usage",
    ]:
        if full[field] != cert[field]:
            raise AssertionError(f"{context}: FULL/CERT {field} differ")
    if not np.allclose(full["final_destroy_weights"], cert["final_destroy_weights"], atol=1e-12, rtol=0.0):
        raise AssertionError(f"{context}: FULL/CERT destroy weights differ")
    if not np.allclose(full["final_repair_weights"], cert["final_repair_weights"], atol=1e-12, rtol=0.0):
        raise AssertionError(f"{context}: FULL/CERT repair weights differ")
...
def benchmark_cell(
    ...
    if iterations > 0:
        ...
        warm_full = run_solver_once(
            instance, capacity, alpha, cache, warm_iterations, warm_seed, "FULL", alns_params
        )
        warm_cert = run_solver_once(
            instance, capacity, alpha, cache, warm_iterations, warm_seed, "CERT", alns_params
        )
    ...
    for repetition in range(repetitions):
        ...
        row = {
            ...
            "identity_pass": 1,
            "trace_identity_pass": 1,
            "solver": "ALNS",
            "accepted_moves": cert["accepted_moves"],
            "accepted_nonimproving": cert["accepted_nonimproving"],
            "new_bests": cert["new_bests"],
            "random_destroy_uses": cert["destroy_usage"][0],
            "worst_destroy_uses": cert["destroy_usage"][1],
            "greedy_repair_uses": cert["repair_usage"][0],
            "regret2_repair_uses": cert["repair_usage"][1],
            "search_trace_digest": cert["search_trace_digest"],
            "destroy_min_frac": alns_params.destroy_min_frac,
            "destroy_max_frac": alns_params.destroy_max_frac,
            "sa_temp": alns_params.sa_temp,
            "sa_cool": alns_params.sa_cool,
            "segment_size": alns_params.segment_size,
            "reaction": alns_params.reaction,
            "sigma_1": alns_params.sigma_1,
            "sigma_2": alns_params.sigma_2,
            "sigma_3": alns_params.sigma_3,
            "local_search": int(alns_params.use_local_search),
        }
        run_rows.append(row)
    ...
    median_accepted_moves = int(round(float(np.median([float(row["accepted_moves"]) for row in run_rows]))))
    median_accepted_nonimproving = int(round(float(np.median([float(row["accepted_nonimproving"]) for row in run_rows]))))
    median_new_bests = int(round(float(np.median([float(row["new_bests"]) for row in run_rows]))))
    median_random_destroy = int(round(float(np.median([float(row["random_destroy_uses"]) for row in run_rows]))))
    median_worst_destroy = int(round(float(np.median([float(row["worst_destroy_uses"]) for row in run_rows]))))
    median_greedy_repair = int(round(float(np.median([float(row["greedy_repair_uses"]) for row in run_rows]))))
    median_regret2_repair = int(round(float(np.median([float(row["regret2_repair_uses"]) for row in run_rows]))))
    ...
    cell_row = {
        ...
        "trace_identity_pass": 1,
        "solver": "ALNS",
        "accepted_moves": median_accepted_moves,
        "accepted_nonimproving": median_accepted_nonimproving,
        "new_bests": median_new_bests,
        "random_destroy_uses": median_random_destroy,
        "worst_destroy_uses": median_worst_destroy,
        "greedy_repair_uses": median_greedy_repair,
        "regret2_repair_uses": median_regret2_repair,
        "destroy_min_frac": alns_params.destroy_min_frac,
        "destroy_max_frac": alns_params.destroy_max_frac,
        "sa_temp": alns_params.sa_temp,
        "sa_cool": alns_params.sa_cool,
...
RUN_FIELDS = [
    ...
    "N", "dimension", "alpha", "capacity", "iterations", "direction", "solver",
    "destroy_min_frac", "destroy_max_frac", "sa_temp", "sa_cool", "segment_size",
    "reaction", "sigma_1", "sigma_2", "sigma_3", "local_search",
    "calls", "coverage", "certified_feasible", "certified_infeasible",
    "fallbacks", "mean_pieces", "mean_route_length_checked", "K",
    "distance", "objective_raw", "accepted_moves", "accepted_nonimproving",
    "new_bests", "random_destroy_uses", "worst_destroy_uses",
    "greedy_repair_uses", "regret2_repair_uses", "search_trace_digest",
    "full_gate_seconds", "cert_gate_seconds", "full_solver_seconds",
    "cert_solver_seconds", "gate_speedup", "solver_speedup",
    "solver_speedup_with_prep", "prep_seconds", "explained_variance",
    "effective_rank", "entropy_rank", "top_share", "residual_cvar",
    "audit_checks", "identity_pass", "trace_identity_pass",
]

CELL_FIELDS = [
    "instance", "source", "profile", "policy", "N", "dimension", "alpha",
    "capacity", "iterations", "repetitions", "direction", "solver",
    "destroy_min_frac", "destroy_max_frac", "sa_temp", "sa_cool", "segment_size",
    "reaction", "sigma_1", "sigma_2", "sigma_3", "local_search",
    "prep_seconds", "explained_variance", "effective_rank", "entropy_rank",
    "top_share", "residual_cvar", "calls", "coverage", "coverage_iqr",
    "certified_feasible", "certified_infeasible", "fallbacks", "mean_pieces",
    "mean_route_length_checked", "K", "distance", "objective_raw",
    "accepted_moves", "accepted_nonimproving", "new_bests",
    "random_destroy_uses", "worst_destroy_uses", "greedy_repair_uses",
    "regret2_repair_uses", "full_gate_seconds", "full_gate_iqr",
    "cert_gate_seconds", "cert_gate_iqr", "gate_speedup", "gate_speedup_iqr",
    "full_solver_seconds", "full_solver_iqr", "cert_solver_seconds",
    "cert_solver_iqr", "solver_speedup", "solver_speedup_iqr",
    "solver_speedup_with_prep", "solver_speedup_with_prep_iqr",
    "audit_checks", "identity_pass", "trace_identity_pass",
]
...
def _validate_resume_row(row: dict[str, object], config: dict[str, object]) -> None:
    checks = [
        ...
        ("solver", str(row.get("solver", "")).upper(), "ALNS"),
        ("segment_size", int(float(row["segment_size"])), int(config["segment"])),
        ("local_search", int(float(row["local_search"])), int(bool(config["local_search"]))),
    ]
    ...
    float_checks = [
        ("destroy_min_frac", float(row["destroy_min_frac"]), float(config["destroy_min"])),
        ("destroy_max_frac", float(row["destroy_max_frac"]), float(config["destroy_max"])),
        ("sa_temp", float(row["sa_temp"]), float(config["sa_temp"])),
        ("sa_cool", float(row["sa_cool"]), float(config["sa_cool"])),
        ("reaction", float(row["reaction"]), float(config["reaction"])),
        ("sigma_1", float(row["sigma_1"]), float(config["sigma1"])),
        ("sigma_2", float(row["sigma_2"]), float(config["sigma2"])),
        ("sigma_3", float(row["sigma_3"]), float(config["sigma3"])),
    ]
...
def parse_args(argv: Sequence[str]) -> dict[str, object]:
    ...
    config: dict[str, object] = {
        ...
        "eps": EPS_FRAC,
        "support": SUPPORT,
        "out": f"go_nogo_orl_cert_alns_{mode}",
        "overwrite": True,
        "resume": False,
        "destroy_min": ALNS_DESTROY_MIN_FRAC,
        "destroy_max": ALNS_DESTROY_MAX_FRAC,
        "sa_temp": ALNS_SA_TEMP,
        "sa_cool": ALNS_SA_COOL,
        "segment": ALNS_SEGMENT_SIZE,
        "reaction": ALNS_REACTION,
        "sigma1": ALNS_SIGMA_1,
        "sigma2": ALNS_SIGMA_2,
        "sigma3": ALNS_SIGMA_3,
        "worst_p": ALNS_WORST_DETERMINISM,
        "local_search": True,
        **DEFAULT_THRESHOLDS,
    }

    integer_keys = {"N", "d", "iters", "reps", "train", "audit_iters", "seed", "segment"}
    float_keys = {
        "cv", "alpha", "eps", "support", "destroy_min", "destroy_max",
        "sa_temp", "sa_cool", "reaction", "sigma1", "sigma2", "sigma3", "worst_p",
    ...
    for argument in arguments:
        ...
        elif key in {"overwrite", "resume", "local_search"}:
    ...
    if int(config["segment"]) <= 0:
        raise ValueError("segment must be positive")
    if not (0.0 < float(config["destroy_min"]) <= float(config["destroy_max"]) < 1.0):
        raise ValueError("destroy fractions must satisfy 0 < min <= max < 1")
    if float(config["sa_temp"]) <= 0.0 or not (0.0 < float(config["sa_cool"]) <= 1.0):
        raise ValueError("sa_temp must be positive and sa_cool in (0,1]")
    if not (0.0 <= float(config["reaction"]) <= 1.0):
        raise ValueError("reaction must lie in [0,1]")
...
def self_test() -> None:
    ...
    params = ALNSParameters(segment_size=2, use_local_search=False)
    run_a = solve_alns(tiny_distance, _AlwaysFeasibleGate(), 6, 4.0, 8, 1234, params)
    run_b = solve_alns(tiny_distance, _AlwaysFeasibleGate(), 6, 4.0, 8, 1234, params)
    if run_a.trace_digest != run_b.trace_digest or canonical_plan(run_a.plan) != canonical_plan(run_b.plan):
        raise AssertionError("deterministic ALNS self-test failed")
    if sum(run_a.destroy_usage) != 8 or sum(run_a.repair_usage) != 8:
        raise AssertionError("ALNS operator-usage self-test failed")

    print(
        "SELFTEST PASS: fractional CVaR, exact O(p log N) max-affine CVaR, "
        "multivariate projection bracket, and deterministic adaptive ALNS"
    )
...
def main() -> None:
    ...
    alns_params = ALNSParameters(
        destroy_min_frac=float(config["destroy_min"]),
        destroy_max_frac=float(config["destroy_max"]),
        sa_temp=float(config["sa_temp"]),
        sa_cool=float(config["sa_cool"]),
        segment_size=int(config["segment"]),
        reaction=float(config["reaction"]),
        sigma_1=float(config["sigma1"]),
        sigma_2=float(config["sigma2"]),
        sigma_3=float(config["sigma3"]),
        worst_determinism=float(config["worst_p"]),
        use_local_search=bool(config["local_search"]),
    )
    alns_params.validate()
    ...
    print("\n" + "=" * 112)
    print("CERTIFIED MULTIVARIATE CVaR GO/NO-GO — full ALNS + exact ORL projection kernel + fallback")
    print("=" * 112)
    print(
    ...
    print(
        "ALNS: destroy=random/worst repair=greedy/regret2 "
        f"frac=[{alns_params.destroy_min_frac:.2f},{alns_params.destroy_max_frac:.2f}] "
        f"SA=({alns_params.sa_temp:g},{alns_params.sa_cool:g}) "
        f"segment={alns_params.segment_size} reaction={alns_params.reaction:g} "
        f"local_search={int(alns_params.use_local_search)}"
    )
