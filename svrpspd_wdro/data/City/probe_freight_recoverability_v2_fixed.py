...
Typical command (Windows CMD / PowerShell):
  python probe_freight_recoverability_v2_fixed.py instance="HANOI-100-1.vrpspd" ^
...
def choose_instance(arg: Optional[str]) -> Path:
    # An explicit path always wins. Strip surrounding quotes defensively because
    # Windows batch/CMD invocations sometimes preserve them in unusual wrappers.
    if arg:
        ...
        raise FileNotFoundError(
            f"Instance not found: {path}. Current directory: {Path.cwd()}"
        )

    # Default to the exact filename used by this dataset.
    preferred = Path("HANOI-100-1.vrpspd")
    if preferred.exists():
        return preferred
    ...
    if not files:
        raise FileNotFoundError(
            "No .vrpspd file found in the current directory; "
            "place HANOI-100-1.vrpspd beside this script or pass instance=<path>"
        )
    if len(files) == 1:
        return files[0]
    raise FileNotFoundError(
        "HANOI-100-1.vrpspd was not found and multiple .vrpspd files exist: "
