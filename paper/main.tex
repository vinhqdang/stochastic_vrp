...
%% EJOR reviews single-blind: the submission carries author names
...
compensation for late customers. Fleets hold two cheaper
...
vehicle's position and prices built from the route's real geometry. We
cast the question of when to exercise which lever as a finite-horizon
optimal-stopping problem under an empirical demand distribution, solved
by
\textsc{Baton}, a policy that estimates per-stop continuation costs by
backward induction over historical scenarios and intervenes exactly
when continuing becomes dearer than the cheapest recourse action. The
policy has no tunable threshold; we prove that no fixed threshold
reproduces the optimal intervention boundary under position-dependent
prices, and that the endpoint-based risk labels of prior work are
blind to mid-route peaks. Across the Dethloff and Salhi--Nagy
benchmarks and new instances on the road networks and real retail
locations of five cities, \textsc{Baton} dominates published recourse
policies---rule-based restocking triggers, depot-return restocking,
rollout, and a reinforcement-learning policy---under six planning
regimes including three published robust planners. It attains
92--99\% of a near-exact dynamic program given fifty times its data,
exceeds the handoff-only clairvoyant bound by pricing the full action
set, adapts its recourse mix to fleet economics without re-tuning, and
runs in milliseconds per route; all underlying routing plans are
certified by mixed-integer programming bounds.
\end{abstract}
...
\begin{tabular}{l p{4.9cm} p{4.6cm}}
\toprule
Label & Policy & Source \\
\midrule
