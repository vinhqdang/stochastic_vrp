...
Proposition~\ref{prop:naive} on synthetic instances, in four
complementary experiments: an accuracy comparison across four
algorithms; a runtime comparison isolating the pseudo-polynomial
(Theorem~\ref{thm:dp}) versus truly polynomial
(Theorem~\ref{thm:fptas}) dependence on the instance's numerical
magnitude; a sensitivity sweep over the FPTAS's accuracy parameter
$\epsilon$; and a robustness check repeating the accuracy comparison
under two alternative instance-generation regimes.
...
submission and are self-contained (no external data was used).
