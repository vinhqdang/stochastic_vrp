...
= \frac{m - k + 1}{m},
\end{equation}
the fraction of the route's $m$ stops still ahead at stop $k$. We flag
...
Equation~\eqref{eq:optsplit} by the same order. We complete the
...
Before turning to results, we record the monitor's cost, since a
detection layer that competes with re-optimization on realized economics
must itself be cheap relative to the re-optimization it gates. Each
event's update (Algorithm~\ref{alg:tempo}) touches one channel and costs
$O(J)$ arithmetic operations to evaluate its mixture bet over a grid of
$J$ points ($J \le 4$ throughout), plus an $O(1)$ EWMA update
(Equation~\eqref{eq:ewma}); the combination statistic
(Equation~\eqref{eq:dualregime}) is $O(K)$ in the number of channels
($K=5$, fixed). State is $O(K)$ words --- one running log-evidence and
one EWMA per channel --- independent of the route length $m$ or the
elapsed horizon $n$, since the monitor is a streaming statistic rather
than one that revisits past events. Pooling a fleet of $F$ vehicles into
one monitor (Section~\ref{sec:study}) does not change this per-event
cost; it only increases the number of events processed over the day to
$O(Fm)$, so total monitor cost over a day is $O(FmJ)$, with $J$ and $K$
small constants. This is negligible next to any of the replanning
backends of Section~\ref{sec:decision}: even the cheapest,
nearest-neighbor-plus-2-opt re-sequencing, is $O(m^2)$ per call, and the
exact and fleet-rebalancing backends are more expensive still, so the
monitor's cost is not the bottleneck of the trigger-plus-replan pipeline
under any backend we evaluate.
...
\textbf{Limitations, collected.} We gather here, in one place, the
limitations flagged individually alongside the results that exposed
them. The sensitivity proxies of Equations~\eqref{eq:sensdemand} and
\eqref{eq:sensrest} are first-order approximations to the true,
fitted-derivative sensitivity of Equation~\eqref{eq:truesens}, chosen
for requiring no additional fitted quantity beyond what the planning
layer already reports; Section~\ref{sec:coupling} states explicitly the
conditions under which this approximation is accurate and the specific
failure mode (a high-penalty, tightly-windowed stop weighted by raw
stop-count rather than by its own economic importance) it does not
capture, since our harness does not model time windows. The channel
validity arguments of Propositions~\ref{prop:validity}--\ref{prop:split}
require the null model's channels to be correctly specified conditional
on the shared covariates the monitor observes (Remark
\ref{rem:independence}); a real fleet's channels may be linked by
unmodeled common factors beyond the single rain covariate this paper
includes, and the martingale guarantee is only as good as that
specification. Theorem~\ref{thm:waiting}'s value-of-waiting result is
proved in a deliberately stylized two-vehicle, single-parameter model
rather than for the fleet-scale, combinatorial rebalancer actually used
in Section~\ref{sec:study}; we verify its qualitative prediction
empirically (Table~\ref{tab:replan}) but do not claim the stylized
model's exact functional form governs the reinsertion heuristic.
Theorem~\ref{thm:growth}'s alternative class $\mathcal P_\Delta$ is a
linearized, Huber-style modeling device rather than a derived
necessity, chosen for tractability in the same spirit as classical
robust-statistics neighborhoods. The Amazon pilot's confidence
intervals (Section~\ref{sec:study}) quantify sampling uncertainty
within the simulated-day design but not the roughly twelvefold
overstatement of independent real-world realizations that design
implies, since the data set supplies one real route geometry per
instance rather than repeated real operating days. None of these
limitations affects the validity guarantees of
Section~\ref{sec:tempo}, which hold given the stated null model
regardless of how well any proxy approximates the unobserved true
sensitivity or true alternative class; they bound how much of the
\emph{power} demonstrated empirically should be attributed to the
specific closed forms in this paper versus to the general coupling
principle they instantiate.
