...
would flip the optimal decision and where it would not. A planning
layer that solves for recourse under an optimal-stopping formulation
--- estimating, by backward induction over historical demand
scenarios, the continuation cost of carrying the current load onward
versus the cheapest available recourse action --- already computes
exactly this local sensitivity as a byproduct of fitting its own value
function. The present paper shows how to read that value function's
local sensitivity back into the design of a monitoring statistic, so
that the same fitted model that tells the fleet \emph{what} to do when
capacity is breached also tells the monitor \emph{where to look}.
...
Section~\ref{sec:related}, closing out the introduction, positions the
paper against dynamic vehicle routing, classical sequential change
detection, and the anytime-valid inference literature.
Section~\ref{sec:model} formalizes the multi-factor stochastic routing
day and its null hypothesis, then builds the master e-process and
proves its validity. Section~\ref{sec:tempo} introduces the
decision-relevant predictable tilts that couple the monitor to the
plan's economics and assembles TEMPO's adaptive betting, dual-regime
combination, and rare-event separation into the final monitor.
Section~\ref{sec:theory} proves the four economic theorems and
describes the pluggable replanning layer they govern.
...
response to accumulated statistical evidence. The more recent
comprehensive survey of \citet{ouertani2024dvrp}, covering work through
early 2023, confirms that this exogenous-clock pattern has not been
displaced by a decade of subsequent methodological progress: re-solve
triggers remain overwhelmingly periodic, event-count-based, or
tied to the arrival of a new customer request, with the re-solve
\emph{algorithm} itself --- adaptive large neighborhood search,
column generation, learned construction heuristics --- being the axis
of innovation rather than the timing decision. \citet{he2026smpc}
...
paper's harness. On the learning side, \citet{chen2025dgta} train a
Dynamic Graph Temporal Attention reinforcement-learning policy that
reacts to time-dependent stochastic travel times end-to-end, with no
explicit change-detection layer and consequently no error-rate
guarantee of any kind; their own baseline for that policy is the
rolling-horizon heuristic of \citet{gmira2021rollingopt}, which we also
reproduce. \citet{kadyrov2025drl} train a graph-neural-network policy
under proximal policy optimization for the closely related setting of
joint demand and traffic uncertainty, again folding the
re-optimization \emph{decision} into the learned policy implicitly
rather than exposing it as a calibrated statistic a dispatcher could
audit or reason about independently of the network's weights. Across
...
auditable error-rate guarantee, which is precisely the gap the present
paper fills.
...
one anytime-valid umbrella, surveyed by \citet{ramdas2023safi} and, in
a comprehensive book-length treatment collating the field's rapid
subsequent growth, by \citet{ramdaswang2025hypothesis}.
...
through matched oracle calibration. A very recent line of work
questions whether e-values are conceptually necessary for anytime
validity at all: \citet{koningvanmeer2026} show that anytime-valid,
optionally-continued sequential tests can be \emph{induced} directly
from a family of classical fixed-sample tests, without an explicit
betting or e-value construction as an intermediate step. This is a
debate about foundations rather than about the guarantee itself --- an
induced sequential test and a test built from an explicit e-process can
coincide numerically --- and we adopt the betting formulation
throughout because it is what makes the coupling of
Section~\ref{sec:tempo} transparent: the sensitivity tilts of
Section~\ref{sec:coupling} are stated most naturally as a modification
of a bet's stake, and Theorem~\ref{thm:growth}'s growth-optimality
argument is native to the betting perspective.
...
and~\ref{sec:theory}: the planning layer's own fitted continuation
costs supply the predictable sensitivity that tells each channel's bet
how hard to push, and Theorem~\ref{thm:growth} shows this choice is not
merely intuitive but growth-optimal against the class of alternatives
that actually move the routing cost.
...
\begin{figure}
\FIGURE
{\includegraphics[width=0.72\textwidth]{figures/tempo_fig1_explainer.png}}
...
probabilistic tool this section invokes --- Ville's inequality, Wald's
identity, the overshoot bounds of \citet{lorden1970excess} and
\citet{woodroofe1982nonlinear}, and elementary convex calculus --- is
classical, and Propositions~\ref{prop:validity}--\ref{prop:split} in
Sections~\ref{sec:eprocess}--\ref{sec:tempo} are, individually, routine
consequences of the tower property and the union bound, stated for
completeness rather than as claims of technical difficulty. We say this
explicitly because a paper that borrows well-established machinery
should not be mistaken for one that reinvents it. What we believe is
genuinely new is not any single probabilistic inequality but the
\emph{composition}: Theorem~\ref{thm:regret} is the first bound, to our
knowledge, that translates a sequential test's detection-delay
guarantee directly into a bound on \emph{realized routing cost} rather
than on sample size or type-I/II error; Corollary~\ref{cor:alphastar}
is the closed-form significance level that composition implies, whose
content is economic (a formula fleet operators can compute from three
quantities they already track) rather than mathematical (the
optimization itself is a two-line convexity argument, and we do not
claim otherwise); Theorem~\ref{thm:waiting} formalizes, in a
deliberately stylized model rather than full generality, why an
evidence-gated trigger can dominate an immediate one when the
downstream decision is itself statistical; and Theorem~\ref{thm:growth}
is the paper's central methodological claim, showing that the
decision-relevant sensitivity weights of Section~\ref{sec:coupling}
--- introduced there as a modeling choice --- are in fact the unique
growth-optimal weights against a natural, if necessarily stylized,
class of cost-relevant alternatives, rather than merely one heuristic
choice among many. Section~\ref{sec:coupling} states, and this section
proves, exactly how heuristic that choice is and is not.
...
decision rule stated entirely in quantities a fleet already measures
--- a per-event staleness cost, an empirical detection rate, and an
...
economics and operations research whenever a convex trade-off between
a logarithmic and a linear term is optimized (the classical
economic-order-quantity formula has the same two-line derivation and
the same practical status: not hard to derive, useful because of what
it lets a manager compute); we make no larger claim for
...
Section~\ref{sec:study}.its achieved split
$\widehat\rho(s) := \rho^\ast(\widehat\mu(s))$ deviates from
Equation~
...
\emph{Tuning protocol, stated explicitly.} TEMPO itself is not tuned
per instance or per scenario: $\alpha = 0.05$, the fixed grids $\Theta$
and the Poisson grid (Section~\ref{sec:eprocess}), the EWMA rate $\beta
= 0.8$ and clip $\theta_{\max} = 2.5$ (Section~\ref{sec:tempo}) are
fixed once across the entire 73-instance, 19-scenario grid; no
per-scenario or per-instance search over these constants was performed.
\emph{Periodic} and \emph{rolling-opt} use conventional, literature-
default cadences (40 events and 5 events respectively) rather than
cadences tuned to this data. \emph{CUSUM}, \emph{Page--Hinkley}, and
the two e-detectors are the only baselines that receive any
data-dependent tuning at all, and it is oracle calibration --- their
threshold is set from 100 held-out null-day Monte Carlo streams to
match TEMPO's own realized false-alarm rate exactly, a procedure no
dispatcher could run without already knowing the answer TEMPO
produces, and one which by construction can only help these baselines'
reported detection rate relative to an untuned deployment of them. No
monitor in the detection-rate comparison receives a materially
different computational budget: every one of them is an $O(1)$-per-
event streaming statistic (Section~\ref{sec:complexity} makes this
precise for TEMPO), so there is no asymmetry in compute time to control
for, only the asymmetry in calibration information just described,
which is resolved in the baselines' favor throughout.
...
\begin{figure}
\FIGURE
{\includegraphics[width=0.62\textwidth]{figures/tempo_fig5_baselines.png}}
...
combination its safety net against single-channel drift.
Section~\ref{sec:ablation} isolates these two components, and the
sensitivity tilt itself, incrementally rather than jointly. A second
...
\subsection{Component ablation: which piece does the work}\label{sec:ablation}
...
The table contains a result we did not anticipate and report because
it is informative rather than because it is flattering: the
sensitivity tilt \emph{alone}, without adaptive betting, is not
uniformly better than no tilt at all --- it \emph{hurts} on the two
demand scenarios (\texttt{demand\_ramp} 0.111 $\to$ 0.040,
\texttt{demand\_severe} 0.351 $\to$ 0.129) before adaptive betting is
added. The mechanism is exactly what Section~\ref{sec:coupling}'s
derivation predicts: the demand sensitivity $s_{\mathrm{demand}}(n)$ is
small whenever the running load sits far from the capacity boundary,
which correctly reflects low decision-relevance, but a \emph{fixed}
grid tilt scaled down by a small $s(n)$ is also a \emph{weaker}
statistical bet in absolute terms, and early-to-mid-route demand drift
generates most of its evidence exactly where $s(n)$ is smallest. Only
once the tilt is allowed to adapt --- learning the drift's true size
online rather than relying on a fixed grid scaled by a proxy --- does
the combination recover and exceed the untilted baseline
(\texttt{tilt\_adapt}: 0.173 and 0.538, both above \texttt{flat}). This
is precisely the sense in which the tilt and the adaptive mechanism are
not independent design choices to be evaluated in isolation: the
sensitivity weighting is only unambiguously beneficial once paired with
a bet that can recover the strength the weighting removes, which the
dual-regime combination then compounds further, especially on
single-channel drift where the mean-isolation term matters most
(\texttt{demand\_severe} \texttt{tilt\_adapt}$\to$\texttt{tempo2}:
$0.538 \to 0.756$, the largest single-step gain in the table, against a
near-ceiling, marginal gain on the already-multi-channel
\texttt{rush\_crush} scenario, $0.991 \to 0.996$, exactly as
Section~\ref{sec:tempo}'s account of when the mean term matters
predicts). On \texttt{traffic\_late} and \texttt{traffic\_mild}, where
travel-time drift is decision-relevant everywhere along the route
(Equation~\eqref{eq:sensrest} is bounded away from zero throughout), the
tilt alone already helps, and adaptive betting and the dual regime
each add a further, smaller increment --- a different, and simpler,
interaction than the demand case, again consistent with the
mechanism rather than merely favorable to it.
...
\subsection{TEMPO complements a capacity-triggered recourse policy, and drift has a where}
...
when a vehicle's running load approaches a capacity boundary. On a
demand-surge day, both layers react: a capacity-triggered recourse
...
policy correctly stays silent, yet the plan's realized lateness is
climbing steadily, exactly the drift TEMPO exists to catch and a
load-based recourse policy has no state to see it with. The two layers
are complementary by design: a capacity-triggered recourse policy
prices \emph{what to do} once a capacity constraint is in play; TEMPO
decides \emph{when} the plan --- any part of it, capacity-related or
not --- has stopped matching reality.
...
not any single route. As the jam pocket of
Section~\ref{sec:model} opens and spreads across the road network, the
evidence pooled from vehicles whose remaining routes enter the affected
zone accumulates while vehicles routed elsewhere continue to contribute
on-model evidence, and the alarm --- fired once the pooled log
e-process crosses its threshold --- triggers a fleet-wide replan well
before the day ends.

\begin{figure}
\FIGURE
{\includegraphics[width=0.72\textwidth]{figures/tempo_fig4_fleet.png}}
...
\begin{figure}
\FIGURE
{\includegraphics[width=0.55\textwidth]{figures/tempo_fig3_map.png}}
...
estimates rather than point estimates alone, computed directly from
the underlying per-route outcomes. Validity holds on real routes
exactly as on synthetic ones: TEMPO's false-alarm rate is $0.018\
[0.010, 0.033]$ on the plain null and $0.022\ [0.013, 0.037]$ on the
forecast-rain null, both within the nominal $\alpha = 0.05$ with the
upper confidence bound comfortably below it, against $0.032\ [0.020,
0.049]$ for oracle-matched CUSUM and $0.037\ [0.024, 0.055]$ for
Page--Hinkley (whose interval's upper end exceeds $\alpha$, consistent
with the theoretical point that its calibration guarantee is generous
but not exact). The discriminating scenario on these industrial-length
...
dramatic single cause --- where TEMPO detects at $0.70\ [0.66, 0.73]$
against oracle-matched CUSUM's $0.23\ [0.20, 0.27]$ and Page--Hinkley's
$0.23\ [0.20, 0.27]$, non-overlapping intervals and a threefold margin,
with the late-onset and ramped demand variants showing the same
non-overlapping pattern ($0.88\ [0.86, 0.91]$ against $0.82\ [0.79,
0.85]$ and $0.23\ [0.20, 0.26]$; $0.95\ [0.93, 0.96]$ against $0.93\
[0.91, 0.95]$ and $0.30\ [0.26, 0.33]$). Traffic scenarios sit at a
shared ceiling for TEMPO and CUSUM alike (0.86--0.97) since 150-stop
routes carry overwhelming travel-time evidence regardless of how it is
combined, while Page--Hinkley collapses on mild traffic drift (0.25);
accident scenarios remain information-limited ties, and compound days
saturate near 0.95 for every monitor. Detection is uniform across
metropolitan areas (0.84--0.86 mean detection in each of the five). We
note two limitations of this pilot for transparency, neither resolved
by the confidence intervals above: the data set is delivery-only, so a
single realization per route stands in for day-to-day dispersion (the
$n=12$ simulated days per route share one real route geometry and one
real set of planner forecasts, so the 600-trial count overstates the
number of independent real-world realizations by roughly a factor of
12), and because the data set records no realized-versus-forecast
traces, drift is still injected rather than observed --- both are
flagged for a future extension that pairs multiple realized days per
route.

\subsection{Retraining DGTA-RL as a fully independent baseline}\label{sec:dgta}

The rolling-opt baseline of Section~\ref{sec:baselines} reproduces the trigger cadence
...
under time-dependent, stochastic travel times, with no explicit
change-detection layer. Because this is an architecturally distinct
comparison --- a learned control policy rather than a statistical
trigger --- we reimplement its dual-attention encoder and
spatial-temporal pointer decoder at reduced scale, train it by
REINFORCE with a frozen baseline network per the original paper's
Algorithm 1, and evaluate its realized tour cost against
TEMPO-triggered re-optimization on matched instances, rather than
citing the architecture without running it. \emph{[Placeholder:
realized-cost comparison to be inserted before submission.]}
...
new fitted model beyond what an optimal-stopping planning layer already
produces for its own recourse decisions --- the continuation cost
$\widehat C(\cdot)$ is read, not re-estimated --- so a fleet that
...
class are first-order and linearized approximations, respectively,
chosen for tractability rather than derived as necessities; the
channel-validity arguments require the null model's channels to be
correctly specified conditional on observed covariates
(Remark~\ref{rem:independence}); and Theorem~\ref{thm:waiting} is
proved in a deliberately stylized model rather than for the
fleet-scale rebalancer actually used. Each limitation is discussed at
the point it arises, and in the Appendix; none of them affects the
validity guarantees of Section~\ref{sec:tempo}, which hold given the
stated null model regardless of how well any proxy approximates the
unobserved truth --- they bound how much of the empirically
demonstrated \emph{power} should be attributed to this paper's
specific closed forms versus to the general coupling principle they
instantiate.
...
\begin{remark}[what Assumption~\ref{ass:growth} does and does not buy]\label{rem:growth}
